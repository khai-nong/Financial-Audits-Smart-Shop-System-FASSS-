...
def search_data():
    ...
    scr_month = cal_1.get_date()
    scr_year = cal_1.get_date()
    db = pymysql.connect(host='localhost', db='fasss', user='root', passwd='yamayan0215')
    cursor = db.cursor()
    sqlString = "SELECT day, SUM(income) AS total_income FROM finance WHERE MONTH(day) = MONTH('%s') AND YEAR(day) = YEAR('%s')" % (scr_month, scr_year)
    cursor.execute(sqlString)
    finance = cursor.fetchall()
    sql_payowe = "SELECT fcst_date, SUM(fcst_pd) AS total_pay ,SUM(fcst_owe) AS total_owe FROM fin_customer WHERE MONTH(fcst_date) = MONTH('%s') AND YEAR(fcst_date) = YEAR('%s')" % (scr_month,scr_year)
    ...
    if finance == ((None, None),):
        ...
        for i, (day, total_income) in enumerate(finance, start=1):
            ...
            month_year = fcst_date.strftime("%B %Y")
            listBox.tag_configure('bold_tag', font=('Leelawadee', 14))
            listBox.insert("", "end", values=(month_year, total_income, total_pay, total_owe, total_2), tags=('bold_tag',))
        for col in cols:
            listBox.heading(col, text=col)
            listBox.place(x=80, y=170)
            listBox.column('เดือนปี', anchor=CENTER, width=200)
            listBox.column('รายได้ในเดือนนี้', anchor=CENTER)
...
def paydebt_data(): #รายชื่อคนชำระหนี้
    paydebt_btn.place_forget()
    lbl_1.place(x=210, y=270)
    scr_month = cal_1.get_date()
    db = pymysql.connect(host='localhost', db='fasss', user='root', passwd='yamayan0215')
    cursor = db.cursor()
    sqlString = "Select * from fin_customer WHERE MONTH(fcst_date) = MONTH('%s') AND fcst_pd != 0 AND YEAR(fcst_date) = YEAR('%s') ORDER BY `fcst_date` ASC" % (scr_month,scr_month)
    cursor.execute(sqlString)
...
def owe_data(): #รายชื่อคนติดหนี้
    owe_btn.place_forget()
    lbl_2.place(x=735, y=270)
    scr_month = cal_1.get_date()
    db = pymysql.connect(host='localhost', db='fasss', user='root', passwd='yamayan0215')
    cursor = db.cursor()
    sqlString = "Select * from fin_customer WHERE MONTH(fcst_date) = MONTH('%s') AND fcst_owe != 0 AND YEAR(fcst_date) = YEAR('%s') ORDER BY `fcst_date` ASC" % (scr_month,scr_month)
    cursor.execute(sqlString)
...
lbl = Label(gui, text="ข้อมูลรายได้ประจำเดือน",bg="#232429",fg="#737988",font= (my_font,37))
lbl.place(x=20, y=15)
lbl = Label(gui, text="เลือกวันที่ต้องการค้นหา : ",bg="#232429",fg="#737988",font= (my_font,20))
lbl.place(x=20, y=80)
cal_1=DateEntry(gui,selectmode='month',fg="#737988",font= (my_font,11))
...
lbl_1 = Label(gui, text="คนที่มาชำระหนี้ในเดือนนี้",fg="#ffffff",font= (my_font,20),bg="#232429")
lbl_2 = Label(gui, text="คนที่มาติดหนี้เพิ่มในเดือนนี้",fg="#ffffff",font= (my_font,20),bg="#232429")
cols = ('เดือนปี', 'รายได้ในเดือนนี้', 'ยอดหนี้ที่ลูกค้าชำระ', 'ยอดหนี้ที่ลูกค้าติดหนี้','รายได้สุทธิ(รวมยอดหนี้ที่ลูกค้าจ่าย)')
...
listBox_pd = ttk.Treeview(gui, columns=paydebt_cols, show='headings', height=9)
listBox_owe = ttk.Treeview(gui, columns=owe_cols, show='headings', height=9)
